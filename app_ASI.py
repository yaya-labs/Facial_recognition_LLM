...
# Get ASI API key from environment variable
ASI_API_KEY = os.environ.get('ASI_API_KEY')

if not ASI_API_KEY:
    print("WARNING: No ASI API key found. Set the ASI_API_KEY environment variable.")
...
# Function to call ASI Mini API
def call_asi_mini(messages, model="asi1-mini", temperature=0, stream=False, max_tokens=512):
...
class FaceManager:
    ...
    def _update_face_summary(self, face_id):
        """Update the summary for a face using ASI Mini API."""
        if not ASI_API_KEY:
            return
...
class ChatApp:
    ...
    def process_message(self, user_message):
        ...
        try:
            ...
            self.logger.error(f"Error in ASI Mini chat: {e}")
...
def main():
    ...
    if not ASI_API_KEY:
        print("WARNING: ASI API key not found. Set ASI_API_KEY environment variable.")
